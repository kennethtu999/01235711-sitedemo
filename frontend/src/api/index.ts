...
export const apiService = {
  ...
  removeProjectUser: (projectId: number, userId: number) => api.delete(`/admin/projects/${projectId}/users/${userId}`),
  removeAllProjectUsers: (projectId: number) => api.delete(`/admin/projects/${projectId}/users`),

  // Hook Log 管理
  getHookLogs: (params?: { page?: number; limit?: number; status?: string; projectId?: number }) =>
    api.get('/hook-logs', { params }),
  getHookLogById: (hookLogId: number) => api.get(`/hook-logs/${hookLogId}`),
  getHookLogStats: (params?: { projectId?: number; days?: number }) => api.get('/hook-logs/stats', { params }),
  reExecuteHookLog: (hookLogId: number) => api.post(`/hook-logs/${hookLogId}/re-execute`),
  ...
  addUserToGroup: (groupId: number, data: AddUserToGroupData) => api.post(`/groups/${groupId}/users`, data),
  removeUserFromGroup: (groupId: number, userId: number) => api.delete(`/groups/${groupId}/users/${userId}`),
